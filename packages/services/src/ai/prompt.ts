...
/**
 * Shared, provider-agnostic prompt construction + JSON extraction helpers.
 * Every concrete AIProvider adapter (anthropic, gemini, …) builds the SAME
 * user message and parses the SAME JSON shape, so swapping providers never
 * changes what the model is asked or how its reply is interpreted. This keeps
 * the anti-hallucination pipeline (system prompt + visibility filtering +
 * GeneratedTextSchema validation) provider-independent.
 */
...
export function extractJson(raw: string): unknown {
  ...
  if (start === -1 || end === -1 || end < start) {
    throw new Error("AIProvider: no JSON object found in response");
  }
